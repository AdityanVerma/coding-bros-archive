...
the coordinator decides to change the problem. 
...
function in_search_of_an_easy_problem(N, opinions)
{
    const opinionsArr = opinions.split(" ").map(Number);
    let flag = 0;

    for (const opinion of opinionsArr)
    {
        if (opinion === 1) {
            flag = 1;
            break;
    ...
    if (flag === 1) console.log("HARD");
    else console.log("EASY");
}

in_search_of_an_easy_problem(3, "0 0 1");
in_search_of_an_easy_problem(1, "0");
in_search_of_an_easy_problem(4, "0 0 0 0");
in_search_of_an_easy_problem(7, "0 0 0 0 1 0 1");
